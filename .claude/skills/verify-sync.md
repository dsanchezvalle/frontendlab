...
3. **Skill behavior contract** — if `AGENTS.md` describes a skill's
   ...
   is wrong somewhere.
4. **Label set consistency** — the canonical labels listed in
   `AGENTS.md → Required labels` must equal the `gh label create` list
   in `setup/github-checklist.md` and the `canonical` array in
   `scripts/verify.sh`. Three sources of truth must agree.
